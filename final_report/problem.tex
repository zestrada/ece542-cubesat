...
satellite with the ground station.  A state machine diagram, provided by the
IlliniSat team, is shown in Figure \ref{fig:state_machine}. The C\&DH system is
based on a MitySOM-335x Processor Card, built with a TI AM335x Application
...
standard \fix{atd} utility (recompiled and modified for this platform) is used
to schedule tasks.
...
\begin{figure}
  ...
  \caption{State machine diagram for IlliniSat-2, courtesy Alex Ghosh and Bindu
  Jagannatha. After launch and deploy, the system starts in the detumble state
  (which can also be reentered in an emergency situation).  In this state,}\label{fig:state_machine}
