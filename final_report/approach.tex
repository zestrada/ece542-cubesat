...
\texttt{word\_sefi\_rate} & SEFI event rate for word errors\footnote{Note that this error rate was so small (even multiplied by the number of words in the system) that we replaced it by 0 with no effect on the reliability result.} & $5.5 \cdot 10^{-15}$ & \cite{Gliem2012memory} \\
