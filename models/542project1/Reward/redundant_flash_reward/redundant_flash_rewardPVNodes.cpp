...
redundant_flash_rewardPV0Worker::redundant_flash_rewardPV0Worker()
{
  NumModels = 1;
...
double redundant_flash_rewardPV0Worker::Reward_Function(void) {
...
redundant_flash_rewardPV0::redundant_flash_rewardPV0(int timeindex) {
  TheModelPtr = (BaseModelClass**)(&Theredundant_flash_modelRJ);
...
redundant_flash_rewardPV0::~redundant_flash_rewardPV0() {
...
void redundant_flash_rewardPV0::CreateWorkerList(void) {
  for(int i = 0; i < NumberOfWorkers; i++)
    WorkerList[i] = new redundant_flash_rewardPV0Worker;
}
